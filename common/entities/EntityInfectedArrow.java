...
public class EntityInfectedArrow extends Entity implements IProjectile {
	private int xTile = -1;
	private int yTile = -1;
	private int zTile = -1;
	...
	public EntityInfectedArrow(World par1World) {
		super(par1World);
		this.setSize(0.5F, 0.5F);
	}

	public EntityInfectedArrow(World par1World, double par2, double par4, double par6) {
		super(par1World);
		this.setSize(0.5F, 0.5F);
	...
	public EntityInfectedArrow(World par1World, EntityLiving par2EntityLiving, EntityLiving par3EntityLiving, float par4, float par5) {
		super(par1World);
		...
		if (this.inGround) {
			...
			if (var18 == this.inTile && var19 == this.inData) {
				...
				if (((BlockTorch)Block.torchWood).canPlaceBlockAt(this.worldObj, nXPos, nYPos, nZPos)/* && this.worldObj.getBlockId(nXPos, nYPos, nZPos) == 0*/)
					this.worldObj.func_94573_a(nXPos, nYPos, nZPos, Block.torchWood.blockID);
				else {
					if (!this.worldObj.isRemote) {
						this.entityDropItem(new ItemStack(ItemsHelper.InfectedArrow, 1), 1);
					}					
				}
					...
					if (var4.entityHit.attackEntityFrom(var21, var23)) {
						if (var4.entityHit instanceof EntityLiving) {
							...
							if (this.knockbackStrength > 0) {
								...
								if (var25 > 0.0F) {
									var4.entityHit.addVelocity(this.motionX * (double) this.knockbackStrength * 0.6000000238418579D / (double) var25, 0.1D, this.motionZ
											* (double) this.knockbackStrength * 0.6000000238418579D / (double) var25);
								}
							}
						}
	...
	public void onCollideWithPlayer(EntityPlayer par1EntityPlayer) {
		if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0) {
			...
			if (this.canBePickedUp == 1 && !par1EntityPlayer.inventory.addItemStackToInventory(new ItemStack(ItemsHelper.InfectedArrow, 1))) {
				var2 = false;
			}
